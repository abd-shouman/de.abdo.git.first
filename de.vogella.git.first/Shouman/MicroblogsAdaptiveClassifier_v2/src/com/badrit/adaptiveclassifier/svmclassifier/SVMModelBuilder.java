...
public class SVMModelBuilder {
	...
	//private static final int MILLISECONDS_IN_HOUR = 1000 * 30; //Milliseconds in 30 sec 
	
	public SVMModelBuilder(long start){
		runBuild(start);
	}
	

	
	public void runBuild(long start) {
		try {
			 ...
			 */
			long negativeCount = getAllNegativeDocsCount(
					startTime);
			 ...
			 * Clear Variables to free up memory 
			 */
			positiveFacets.clear();
			positiveFacets = null;
			for(HashSet<Long> value: termsInvertedIndex.values()){
				value.clear();
				value = null;
			}
			termsInvertedIndex.clear();
			termsInvertedIndex = null;
			System.gc();
			 ...
			 * Clear Variables to free up memory 
			 */
			reverseFeaturesMap.clear();
			expansionList.clear();
			reverseFeaturesMap = null;
			expansionList = null;
			System.gc();
			...
			expansionSet.clear();
			expansionSet = null;
			positiveTweetsText.clear();
			positiveTweetsText = null;
			negativeTweetsText.clear();
			negativeTweetsText = null;
			System.gc();
			System.out.println("Done Clearing Variables");
			...
			System.err
					.println("##SVM-Builder: error in SVM Builder");
			e.printStackTrace();
		}
	...
	private long getAllNegativeDocsCount(
		...
		for(int file=(Program.fileToRead-(Program.initialWaitingTime/Program.frequentGapTime)); file < Program.fileToRead; file++){
			...
			while ((entry = br.readLine()) != null) {
				timeStamp = Long.parseLong(entry.split("\t")[2].replace("\ufeff", ""),10);
				if((timeStamp - startTime)/(3600*1000) > Program.initialWaitingTime)
					continue;
				docCount++;
			}
			br.close();
		...
		for(int file=(Program.fileToRead-(Program.initialWaitingTime/Program.frequentGapTime)); file < Program.fileToRead; file++){
			...
			while ((entry = br.readLine()) != null) {
				...
				if(docCount%100000 == 0){
					System.out.println("#ngeativeDocCount = " + docCount + "\n" + 
							"#remaining = " + ((positiveDocsCount * SVM_NEGATIVE_TRAIN_MULTIPLE_OF_POSITIVE) - docCount + "\n") );
				}
				
				constructTermFacets(entry.split("\t")[1], negativeFacets);
				constructInvertedIndex(entry.split("\t")[1], Long.parseLong( entry.split("\t")[0].replace("\ufeff", "") , 10),
						termsInvertedIndex);
				negativeTweetsText.add(entry.split("\t")[1]);
			}
			br.close();
		}
		return docCount;
		...
		BufferedWriter bw = new BufferedWriter(
				new OutputStreamWriter(
						new FileOutputStream(
								new File(
										"ExpansionList/expansionList_model_V"+ Program.modelVersion +".txt")),
						"UTF8"));
		for (String expansionTerm : expansionSet)
			bw.write(expansionTerm + "\n");
		int modelVersion = Program.modelVersion;
		bw.close();
	}
	...
	private long constructPositiveFacetCountAndInvertedIndex(
		...
		for(int file=(Program.fileToRead-(Program.initialWaitingTime/Program.frequentGapTime)); file < Program.fileToRead; file++){
			String classifierOutputFilePath = "boolean_positive_tweets_v" + file + ".txt"; 
			br = new BufferedReader(new InputStreamReader(
			...
			while ((entry = br.readLine()) != null) {
				if((Long.parseLong(entry.split("\t")[2].replace("\ufeff", ""),10) - startTime)/(3600*1000) > Program.initialWaitingTime)
					continue;
				docCount++;
				constructTermFacets(entry.split("\t")[1], facets);
				constructInvertedIndex(entry.split("\t")[1], Long.parseLong( entry.split("\t")[0].replace("\ufeff", "") , 10), invertedIndex);
				positiveTweetsText.add(entry.split("\t")[1]);
			}
			br.close();
		}
		return docCount;
	...
	private void constructInvertedIndex(String tweetText, long tweetID,
			HashMap<String, HashSet<Long>> invertedIndex) {
		for (String token : tweetText.split(" +")) {
	...
	private void constructTermFacets(String tweetText,
		...
		for (String token : tweetText.split(" +")) {
			if (!facets.containsKey(token))
				facets.put(token, 0l);
			facets.put(token, facets.get(token) + 1);
		}
	}
}